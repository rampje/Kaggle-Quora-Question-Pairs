...
This blog [post](http://blog.christianperone.com/2011/09/machine-learning-text-feature-extraction-tf-idf-part-i/) provides an introduction to [**term frequency-inverse document frequency** (**tf-idf**)](https://en.wikipedia.org/wiki/Tf%E2%80%93idf) and the [**Vector Space Model** (**VSM**)](https://en.wikipedia.org/wiki/Vector_space_model). These concepts provide the mathematical foundations which motivate using cosine similarity to determine whether texts are the same.
...
bt_df1 <- train[c("is_duplicate","char_diff","cos_sims","q1_wc","q2_wc")]
...
gbmGrid <- expand.grid(
  ...
  eta = 0.3, # shrinkage / learning rate
...
```{r}
gbt1_preds <- predict(gbmTree1 , newdata = test, type = "prob")
```
