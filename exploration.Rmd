...
```{r}
t1 <- Sys.time()
test_cos_sims <- cosine_similarity(test$question1, test$question2)
t2 <- Sys.time()
t2-t1
```

```{r}
test$cos_sims <- test_cos_sims
```
...
```{r}
gbt1_preds <- predict(gbmTree1 , newdata = na.omit(test), type = "prob")
gbt1_preds$test_id <- na.omit(test)$test_id
head(gbt1_preds)
```
...
```{r}
submission1 <- gbt1_preds[c("test_id","1")]
names(submission1)[2] <- "is_duplicate"
head(submission1)
```
